...
export function TaskDraftReview({
  ...
  drafts: SelectableTaskDraft[];
  onChange: (drafts: SelectableTaskDraft[]) => void;
}) {
  function patch(index: number, change: Partial<SelectableTaskDraft>) {
      ...
      {drafts.map((draft, index) => (
        <Card key={index}>
          <Pill
            ...
            onPress={() => patch(index, { selected: !draft.selected })}
          />
          <Input
          ...
          <Input
            ...
            maxLength={10}
            onChangeText={(dueDate) => patch(index, { dueDate: dueDate || null })}
          />
