...
export const voiceAdapter: VoiceAdapter = {
  ...
  async prepare(pack, signal, onProgress) {
    ...
    if (pack === 'speech') {
      speechModel = await runtime.download(runtime.models.speechToText.WHISPER.TINY.XNNPACK_FP32, {
        signal,
        onProgress,
      });
    } else {
      ...
      outputModels[pack] = await runtime.download(
        { ...selected, voices: { [voices[pack]]: selected.voices[voices[pack]]! } },
        { signal, onProgress },
      );
    }
  },
  async requestPermission() {
    ...
    if ((await AudioManager.requestRecordingPermissions()) !== 'Granted')
      throw new AIInterruptedError(
        'Microphone access is off. Enable it for Saathi in your phone settings to record.',
      );
  },
  async capture(onAudio, onError, signal) {
    ...
    const interruption = AudioManager.addSystemEventListener('interruption', ({ type }) => {
      if (type === 'began')
        onError(
          new Error(
            'Audio was interrupted. Review the completed transcript and start a new recording.',
          ),
        );
    });
    const route = AudioManager.addSystemEventListener('routeChange', ({ reason }) => {
      if (reason === 'OldDeviceUnavailable' || reason === 'NoSuitableRouteForCategory')
        onError(
          new Error('The microphone connection changed. Reconnect it and start a new recording.'),
        );
    });
    let stopped = false;
    ...
    try {
      recorder.onError(() =>
        onError(new Error('The microphone stopped unexpectedly. Try recording again.')),
      );
      const callback = recorder.onAudioReady(
        { sampleRate: VOICE_SAMPLE_RATE, bufferLength: 1600, channelCount: 1 },
        ({ buffer, numFrames }) => {
          if (stopped || signal.aborted) return;
          if (buffer.sampleRate !== VOICE_SAMPLE_RATE || buffer.numberOfChannels !== 1) {
            onError(
              new Error(
                'This audio route cannot provide 16 kHz mono audio. Try the phone microphone.',
              ),
            );
            return;
          }
  ...
  async speech(transcription) {
    ...
    const { createWhisperSpeechToText, createFsmnVoiceActivityDetector } =
    ...
    try {
      if (transcription) stt = await createWhisperSpeechToText(speechModel);
    } catch (error) {
      vad.dispose();
      throw error;
    }
    return {
      ...
      transcribe: async (audio, language, onToken) =>
        stt ? stt.transcribe(audio, { language }, onToken) : '',
  ...
  async output(language) {
    ...
    if (!config)
      throw new AIInterruptedError('Download the selected read-aloud voice in Settings first.');
    ...
    return {
      synthesize: (text, speed) =>
        tts.synthesize(text, { voice: voices[language], speed, maxChunkLength: 180 }),
  ...
  async play(audio, sampleRate, signal) {
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const done = (error?: Error) => {
          ...
          if (started && error) {
            try {
              ...
              /* Audio may already have ended after an interruption. */
            }
          }
        ...
        const route = AudioManager.addSystemEventListener('routeChange', ({ reason }) => {
          if (reason === 'OldDeviceUnavailable') abort();
        });
        AudioManager.observeAudioInterruptions('gainTransient');
        ...
        try {
          ...
          done(new Error('Audio playback could not start.'));
        }
      });
    } finally {
      source.disconnect();
