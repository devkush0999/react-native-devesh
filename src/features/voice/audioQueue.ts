...
export class AudioQueue {
  ...
  get pendingSeconds() {
    return this.size / VOICE_SAMPLE_RATE;
  }
  push(frame: Float32Array) {
    if (this.closed) return;
    if (this.size + frame.length > this.maxSamples)
      throw new Error(
        'This phone could not keep up with live audio. Recording stopped; review the completed transcript and try shorter clips.',
      );
