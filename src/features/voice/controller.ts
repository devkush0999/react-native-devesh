...
export type VoiceState = {
  supported: boolean;
  status: VoiceStatus;
  ready: Record<VoicePack, boolean>;
  progress: number;
  stage: string;
  error: string | null;
  notice: string | null;
...
const insightSchema = z.object({
  ...
  tasks: z.array(taskDraftSchema).max(8),
});
...
const stopped = () =>
  new AIInterruptedError('Voice stopped. Completed text is available for review.');
function assertRunning(context: OperationContext) {
  if (context.signal.aborted) throw stopped();
}
...
export class VoiceController {
  ...
  constructor(
    ...
    private engine: Pick<LocalAIEngine, 'runOperation' | 'getSnapshot' | 'cancel'>,
  ) {
    this.state = {
      supported: adapter.supported,
      status: 'idle',
      ready: { speech: false, en: false, hi: false },
      progress: 0,
      stage: '',
      error: null,
      notice: null,
  ...
  private async perform(status: VoiceStatus, job: () => Promise<void>) {
    ...
    try {
      ...
      this.update({
        error: error instanceof Error ? error.message : 'Voice could not finish. Please retry.',
      });
    } finally {
  ...
  prepare = async (pack: VoicePack) => {
    await this.perform('downloading', async () => {
      if (!this.adapter.supported)
        throw new Error('Voice models require a native development build.');
      await this.engine.runOperation('setup', async (context) => {
        context.phase('download');
        this.update({
          progress: 0,
          stage: `Downloading ${pack === 'speech' ? 'Whisper + speech detection' : pack === 'en' ? 'English voice' : 'Hindi voice'}`,
        });
        await this.adapter.prepare(pack, context.signal, (progress) =>
          this.update({ progress: Math.min(1, Math.max(0, progress)) }),
        );
  ...
  record = async (mode: VoiceMode, language: VoiceLanguage) => {
    ...
    await this.perform('permission', async () => {
      ...
      if (
        mode === 'assistant' &&
        (!this.engine.getSnapshot().available || !this.state.ready[language])
      )
        throw new Error(
          'Voice assistant needs the text AI model and the selected read-aloud voice. Set these up in Settings first.',
        );
      ...
      this.update({ status: 'loading', stage: 'Checking device health before capture' });
        ...
        if (mode === 'assistant' && this.state.transcript.trim()) {
          ...
          const answer = visibleAnswer(
            await context.generate(
              `${systemPrompt(language === 'hi' ? 'Hindi' : 'English')} Use no more than 120 words for spoken playback.`,
          ...
          if (!answer || answer.length > 3000)
            throw new AIInterruptedError(
              'The voice answer was empty or too long. Try a shorter question.',
            );
          this.update({ answer });
  ...
  private async capture(context: OperationContext, mode: VoiceMode, language: VoiceLanguage) {
    ...
    const stopCapture = () => {
      ...
      stopping = (async () => {
        try {
          ...
          failure = new Error(
            'The microphone did not close cleanly. Start a new recording to retry.',
          );
        } finally {
          acceptingAudio = false;
          queue.close(context.signal.aborted);
        }
      })();
    };
    ...
    try {
      this.update({
        status: 'loading',
        stage:
          mode === 'activity' ? 'Loading speech detector' : 'Loading Whisper + speech detector',
      });
      context.phase('loading');
      speech = await this.adapter.speech(mode !== 'activity');
      assertRunning(context);
      microphone = await this.adapter.capture(
        (audio) => {
          ...
          this.update({
            durationSeconds: inputSamples / VOICE_SAMPLE_RATE,
            levels: [...this.state.levels.slice(-31), level],
            queuedSeconds: queue.pendingSeconds,
          });
          if (frame.length) {
            try {
              queue.push(frame);
          ...
          if (inputSamples >= limit) {
            this.update({
              notice: `${mode === 'assistant' ? 'One-minute' : 'Ten-minute'} recording limit reached. Review this clip before starting another.`,
            });
            stopCapture();
          }
        },
      ...
      if (finalizing || context.signal.aborted) {
        await microphone.stop();
        assertRunning(context);
      }
      this.finishCapture = stopCapture;
      if (!finalizing) this.update({ status: 'recording', stage: 'Listening on your device' });
      context.phase('listening');
      watchdog = setTimeout(
        () => {
          this.update({ notice: 'Recording time limit reached.' });
          stopCapture();
        },
        (mode === 'assistant' ? 60 : MAX_VOICE_SECONDS) * 1000,
      );
      ...
      const transcribeSegment = async () => {
        ...
        let lastUpdate = 0;
        const text = (
          await speech!.transcribe(joinAudio(segment), language, (token) => {
            partial += token;
            if (!context.signal.aborted && Date.now() - lastUpdate > 120) {
              this.update({ draft: partial });
              lastUpdate = Date.now();
            }
          })
        ).trim();
        ...
        if (transcript.length > MAX_TRANSCRIPT_LENGTH)
          throw new AIInterruptedError(
            'Transcript limit reached. Save the completed text and start another clip.',
          );
      ...
      for await (const frame of queue.read()) {
        ...
        assertRunning(context);
        const voiced = Math.min(
          seconds,
          spans.reduce((sum, span) => sum + Math.max(0, span.end - span.start), 0),
        );
        this.update({
          speechDetected: spans.length > 0,
          speechSeconds: this.state.speechSeconds + voiced,
          queuedSeconds: queue.pendingSeconds,
        });
        if (mode === 'activity') continue;
        if (spans.length || segment.length) {
          if (!segment.length && preroll) {
            segment.push(preroll);
            segmentSeconds += preroll.length / VOICE_SAMPLE_RATE;
          }
          segment.push(frame);
      ...
      if (mode !== 'activity' && !this.state.transcript.trim())
        this.update({
          notice:
            'No speech was transcribed. Try a quieter place and speak closer to the microphone.',
        });
    } finally {
  ...
  summarize = async (transcript: string, language: VoiceLanguage) => {
    await this.perform('thinking', async () => {
      if (!transcript.trim() || transcript.length > MAX_TRANSCRIPT_LENGTH)
        throw new Error('Add a transcript of up to 24,000 characters first.');
      await this.engine.runOperation('inference', async (context) => {
        ...
        for (let i = 0; i < chunks.length; i++) {
          assertRunning(context);
          this.update({ stage: `Reviewing transcript part ${i + 1} of ${chunks.length}` });
          const raw = await context.generate(
            systemPrompt(language === 'hi' ? 'Hinglish' : 'English'),
          ...
          try {
            ...
            throw new AIInterruptedError(
              'The model returned an invalid meeting summary. Your transcript is unchanged. Try summarizing a shorter excerpt.',
            );
          }
        }
        ...
        for (let pass = 0; summary.length > 2000 && pass < 4; pass++) {
          ...
          for (const chunk of splitTranscript(summary)) {
            assertRunning(context);
            this.update({ stage: 'Combining the meeting summary' });
            reduced.push(
              visibleAnswer(
                await context.generate(
                  systemPrompt(language === 'hi' ? 'Hinglish' : 'English'),
                  `Condense these meeting notes to at most 80 words, preserving decisions and uncertainties: ${JSON.stringify(chunk)}`,
                  () => {},
                ),
              ),
            );
        ...
        assertRunning(context);
        if (!summary.trim() || summary.length > 3000)
          throw new AIInterruptedError('The summary is too long. Try a shorter transcript.');
  ...
  speak = async (text: string, language: VoiceLanguage, speed: number) => {
    await this.perform('loading', async () => {
      if (!text.trim() || text.length > MAX_TRANSCRIPT_LENGTH)
        throw new Error('Read-aloud accepts up to 24,000 characters.');
      if (!this.state.ready[language])
        throw new Error('Download the selected read-aloud voice in Settings first.');
      await this.engine.runOperation('voice', (context) =>
        this.speakWithin(context, text, language, speed),
      );
    });
  };
  private async speakWithin(
    ...
    try {
      ...
      for (const part of splitTranscript(text, 600)) {
        ...
        for await (const chunk of output.synthesize(part, Math.min(1.5, Math.max(0.75, speed)))) {
          assertRunning(context);
          this.update({ status: 'speaking', stage: 'Reading aloud on your device' });
